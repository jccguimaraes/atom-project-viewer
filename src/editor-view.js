...
const typeContainer = function _typeContainer(parentView) {
	...
	if (context.candidate && context.candidate.hasOwnProperty('type')) {
		return;
	}
	if (!context.candidate && context.model) {
		return;
	}
...
const nameContainer = function _setName(parentView) {
	...
	var placeholderText = "Project Name";

	nameInput.placeholder = checkIfGroup(context) ? "Group Name" : "Project Name";
...
function checkIfGroup(ctx){
	var amGroup;
	if(ctx) {
		if (ctx.candidate ){
			if(ctx.candidate.type === 'group'){
				amGroup = 1;
			}
		}
		if( ! amGroup ){
			if ( ctx.model ){
				if(ctx.model.type === 'group'){
					amGroup = 1;
				}
			}
		}
...
const sortByContainer = function _sortByContainer(parentView) {
	...
	line.classList.add("sortby-line");
	...
	lineSpan.textContent = "Sort by: ";

	line.appendChild(lineSpan);
	line.appendChild(sortBySelect);
	sortByBlock.appendChild(line);
...
const iconContainer = function _iconContainer(parentView) {
	...
	// const iconHeader = buildHeader('Icons'); // mike
	const iconInput = buildInput('search', 'icon');
	iconInput.placeholder = "Search Icons";
	...
	// iconBlock.appendChild(iconHeader); // mike
	iconBlock.appendChild(iconList);
	iconBlock.appendChild(iconInput); // mike search below.

	parentView.appendChild(iconBlock);
};

const clickCustomColor = function _clickCustomColor(context, evt) {
	if (evt.target.parentNode.hasAttribute('hidden')) {
		return;
	}
...
const colorContainer = function _colorContainer(parentView) {
	...
	//const colorHeader = buildHeader('Color'); // mike
	...
	//colorPaletteText.textContent = 'quick picker' //@commit: mike less is more
	...
	//colorBlock.appendChild(colorHeader); // mike
...
const groupsContainer = function _parentContainer(parentView) {
	...
	const groupsHeader = buildHeader('Group');

	context.refs['pv-container-groups'] = groupsBlock;

	const listTree = document.createElement('ul');
	listTree.classList.add('list-tree', 'groups-list', 'input-text');
	...
	if(hasSome){
		...
		//groupsBlock.appendChild(title);
		groupsBlock.appendChild(listTree);
		var info = document.createElement("div");
		info.textContent = "(Hint: You can also drag-drop to arrange items in groups in the project drawer.)";
		groupsBlock.appendChild(info);
...
const initialize = function _initialize(model, candidate) {
	...
	const panelHeading = document.createElement('h1');
	const panelBody = document.createElement('div');
	const colWrap = document.createElement('div');
	const col_L = document.createElement('div');
	const col_R = document.createElement('div');

	col_L.classList.add('panel-body-col');
	col_R.classList.add('panel-body-col');
	colWrap.classList.add('panel-body-col-wrap');

	panelHeading.classList.add('panel-head');
	panelBody.classList.add('panel-body');

	panelBody.appendChild(panelHeading);
	nameContainer.call(this, col_L);
	pathsContainer.call(this, col_L); // @commit: mike: Put path and name up top... all the other stuff is candy.
	typeContainer.call(this, col_L);
	sortByContainer.call(this, col_L);
	groupsContainer.call(this, col_L);
	optionsContainer.call(this, col_L);
	configContainer.call(this, col_L);
	actionsContainer.call(this, col_L);
	colorContainer.call(this, col_R);
	iconContainer.call(this, col_R);
	...
	colWrap.appendChild(col_L);
	colWrap.appendChild(col_R);
	panelBody.appendChild(colWrap);
	...
	if (
		...
		clickType.call(this, context, {
			target: context.refs['pv-type-group']
		});
	} else {
		context.refs['pv-type-project'].checked = true;
		clickType.call(this, context, {
			target: context.refs['pv-type-project']
		});
	}
};
