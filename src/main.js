...
const commandsCore = function _commandsCore() {
	return {
		'core:move-up': function() {
			return this.traverse.call(this, '☝️');
		},
		'core:move-down': function() {
			return this.traverse.call(this, '👇');
		},
		'core:move-left': function() {
			return this.setAction.call(this, '📪')
		},
...
const observePathsChanges = function _observePathsChanges(paths) {
	if (database.pathsChangedBypass) {
		return;
	}
...
const createNewProject = function _createNewProject(evt) {
	...
	var existingPaths = [];
	var existing = database.fetch();
	for (var i = 0; i < existing.length; i++) {
		var paths = existing[i].paths;
		if (paths && paths.length) {
			existingPaths = existingPaths.concat(paths);
		}
	}

	//console.log("projectsListView", this.projectsListView.selectListView.items[0]);
	// project's list
	...
	if (dirs && dirs.length) {
		...
		if (~existingPaths.indexOf(tpath)) {
			isCopy = true;
		}
	...
	// Revert to blank new project (not auto populated)
	if (isCopy) {
		tpathArray = null;
		name = "NEW_PROJECT";
	}
