...
class ProjectView extends HTMLElement {
    ...
    setPaths () {
        ...
        this.addEventListener('click', function clickProject(event) {
            ...
            if (this.project.buffers) {
                while (atom.project.buffers.length > 0) {
                    atom.project.removeBuffer(
                        atom.project.buffers[atom.project.buffers.length - 1]
                    );
                }
                this.project.buffers.forEach(function forEachBuffer(buffer) {
