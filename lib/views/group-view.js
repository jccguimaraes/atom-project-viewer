...
class GroupView extends HTMLElement {
    ...
    setName () {
        ...
        if (/(^#[0-9A-F]{6}$)|(^#[0-9A-F]{3}$)/i.test(this.group.color)) {
            ...
            this.docSheet = document.querySelector('style[source-path="' + atom.packages.packageDirPaths[0] + '/project-viewer/styles/project-viewer.less"]');
            if (!this.docSheet.sheet) {
                this.docSheet = document.querySelector('style[source-path="' + atom.styles.configDirPath + '/packages/project-viewer/styles/project-viewer.less"]');
            }
            if (this.docSheet.sheet) {
                this.docSheet.sheet.insertRule(this.CSSRule, this.docSheet.sheet.rules.length);
            }
        }
