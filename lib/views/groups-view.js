...
class GroupsView extends HTMLElement {
    ...
    createdCallback () {
        ...
        function addProjectNode(data) {
            let p,
                projectView;
            for (p = this.children.length - 1; p >= 0; p--) {
                if (
                    this.children[p].children[0] && this.children[p].children[0].lastChild &&
                    this.children[p].children[0].lastChild.textContent.toLowerCase() === data.object.group.toLowerCase()) {
                    break;
                }
            }
            ...
            this.children[p].children[1].appendChild(projectView);
        }
