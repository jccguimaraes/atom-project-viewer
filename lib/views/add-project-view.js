...
const GroupSelectionView = require('./group-selection-view');
const PathsSelectionView = require('./paths-selection-view');
...
class AddProjectView extends HTMLElement {
    ...
    constructProject() {
        let project = {};
        project.name = this.inputView.querySelector('* /deep/ .text').textContent;
        project.group = this.groupSelect.select.selectedOptions[0].textContent;
        project.paths = [];
        for (var i = 0; i < this.pathsContainer.select.options.length; i++) {
            project.paths.push(this.pathsContainer.select.options[i].textContent);
        }

        DataBase.file.emitter.emit('updated', {type: 'projects', action: 'add', object: project});
