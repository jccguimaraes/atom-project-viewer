...
class AddProjectView extends HTMLElement {
    ...
    constructProject () {
        ...
        project = {};
        nameRef = this.inputView.querySelector('* /deep/ .text');
        if (!nameRef) {
            return false;
        }
        project.name = nameRef.textContent;
        ...
        DataBase.file.emitter.emit(Constants.events.updated, {type: 'projects', action: 'add', object: project});
        return true;
    }
