...
class DataBase {
        ...
        function updated(data) {
            ...
            if (data.action === 'add' && !obj.length) {
                ...
                DataBase.data[data.type].sort(function sortData(current, next) {
                    if (!current.hasOwnProperty('name') || !next.hasOwnProperty('name')) {
                        return 0;
                    }
                ...
                if (data.edited) {
                    ...
                    DataBase.data.projects.forEach(function forEachProject(project) {
                        if (this.edited && this.object.name === project.group) {
                            for (let editedProp in this.edited) {
                                if (editedProp === 'name') {
                                    project.group = this.edited[editedProp];
                                } else {
                                    this.object[editedProp] = this.edited[editedProp];
                                }
                            }
                        }
                    }, data);
    ...
    sanitize (data) {
        if (data && typeof data !== 'string' && !Array.isArray(data) && Object.keys(data).length === 2 &&
            ...
            let newData;
            try {
                newData = JSON.parse('{' + data + '}') || JSON.parse(data);
